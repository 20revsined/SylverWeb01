...
class SylverVideoCall(SylverVideoCallTemplate):
  ...
  def MakeVideoCall(self):
    maximum = self.MaxParticipants()
    chat = self.ChatEnabled()
    name = self.VideoCallName()
    response = self.IsPublic()
    anvil.server.call('CreateVideoCall', maximum, chat, name, response)
...
#https://anvil.works/docs/uplink/quickstart, https://anvil.works/docs/uplink, https://anvil.works/docs/server
